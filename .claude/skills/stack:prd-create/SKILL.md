...
**Do not use when** the user wants to *review* an existing PRD — defer to `stack:prd-reviewer`.
...
user as a **multi-select** (pre-select the best match inferred from the feature ask): *"Which
product(s) is this PRD for? Select the most relevant product knowledge module(s)."* Install any
...
first, review second**. Each single-select's **menu rows are the *found* skills plus native**, ranked:
...
review). Pre-select the highest-ranked *found* row (1 → 2 → native); omit any channel row that found
...
*only* when native would otherwise be the sole row (neither channel found a skill); when a skill is
...
**0c. Orient & infer depth.** Load only the cheap *always-read* orientation for the primary product
...
into *this session's* grounding context. If `synced_at` is **< 1 day old**, skip the top-up. **Never
...
**Scope every lookup to *this* PRD** —
...
**Consult code context (C) when necessary** — to verify *actual* behavior, check feasibility, or
...
capability's *Intent & roadmap* facet (tagged `[src: prd:<feature-slug>]`). End with the handoff line:
