...
```
If ngrok isn't running, start it (`ngrok http 8881`) and set the `DEV_LOCAL` `a11yEngineHost` (per the table) to the current `*.ngrok-free.app` host. A build with a stale/missing host still *succeeds* — it just won't connect. Flag it.
