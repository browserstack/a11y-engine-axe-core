...
export function truncateElement(element) {
  ...
  if (str.length < maxLen) {
    ...
    for (const { name, value } of elementNodeMap) {
      // Skip data-percy- attributes
      if (name.startsWith('data-percy-')) {
        console.log(
          `Skipping attribute ${name} for element ${nodeName} in truncateElement`
        ); // Debug log
        continue;
      } else {
        console.log(
          `Including attribute ${name} for element ${nodeName} in truncateElement`
        );
      }
