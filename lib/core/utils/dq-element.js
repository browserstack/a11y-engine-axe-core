...
// a11y-critical : change for runTypeAOpt
...
export function truncateElement(element) {
  ...
  // a11y-critical : Skip data-percy- attributes
  ...
  // a11y-critical : Skip data-percy- attributes for accurate comparison later
  const validAttributeCount = Array.from(elementNodeMap).filter(
    ({ name }) => !name.startsWith('data-percy-')
  ).length;

  let str = getOuterHtml(shallowNode);

  // a11y-critical : Skip data-percy- attributes from shallow node HTML
  ...
  // a11y-critical : Skip data-percy- attributes for accurate comparison later - if there are more valid attributes than what we could include, indicate that with ...
  else if (attributeStrList.length < validAttributeCount) {
...
/**
 * a11y-critical : Escapes a string for use in CSS selectors
...
// a11y-critical : funtion to generate selector with shadow DOM support
...
// a11y-critical : function to get shadow DOM selector
...
// a11y-critical : function to get full path selector
...
// a11y-critical : optimized getSource function with caching
...
const DqElement = memoize(function DqElement(elm, options, spec) {
  ...
  if (!axe._audit.noHtml) {
    // a11y-critical : Differentiate source generation based on runTypeAOpt
...
DqElement.prototype = {
   ...
   */
  get selector() {
    // a11y-critical : change for ancestry
    if (axe._cache.get('targetFormat') === 'ancestry') {
      return this.spec.selector || [getAncestry(this.element)];
    }
    // a11y-critical : Differentiate selector generation based on runTypeAOpt
    if (axe._cache.get('runTypeAOpt')) {
