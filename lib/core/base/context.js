...
function pushUniqueFrame(context, frame) {
  // [a11y-critical]: COI ad denylist — this is the tag-based frame-enumeration
  // path (select('frame, iframe')) used by a full-page axe.run(document), and is
  // the twin of the selector path gated in parse-selector-array.js. Both push
  // sites MUST drop denied frames, otherwise a frame skipped by one path is
  // re-added by the other (findBy dedup only matches frames already pushed). Read
  // the Set injected via axe.configure; undefined when the Type A cross-origin
  // flag is off => enumeration is byte-identical to main. See spec §5/§5.1.
  ...
  if (
    ...
    isDeniedFrameOrigin(frame, crossOriginDenylist)
  ) {
    return;
  }
