...
export default function run(...args) {
  ...
  if (options.advancedExecution) {
    /**
     * Quit flag is used to terminate axe if socket connection can't be established.
     * Otherwise we register a terminateAxe function in cache to be used when socket is
     * disconnected during advanced rule execution.
     */
    if (axe.quitFlag === true) {
      axe.teardown();
      errorRunRules('Socket connection failed');
    } else {
      axe._cache.set('terminateAxe', err => {
        axe.teardown();
        errorRunRules(err);
      });
    }
