...
export default function colorContrastEvaluate(node, options, virtualNode) {
  window.debuglogs.push('>>> AXE colorContrastEvaluate called');
  const {
    ignoreUnicode,
    ignoreLength,
    ignorePseudo,
  ...
  if (!isVisibleOnScreen(node)) {
    window.debuglogs.push('> element not visible on screen');
  ...
  if (
    ...
    window.debuglogs.push(`> element contrast : ${contrast}`);
  ...
  window.debuglogs.push(`> axe isValid : ${isValid}`);
  return isValid;
}
