...
export default function accessibleTextVirtual(virtualNode, context = {}) {
  ...
  const b1Memo =
    ...
    !context.ignoreIconLigature &&
    !context.ignoreNativeTextAlternative &&
    !!virtualNode._cache;
  // [a11y-core]
  const b1Seen =
    b1Memo &&
    context.processed instanceof Set &&
    context.processed.has(virtualNode);
  if (b1Memo && !b1Seen && virtualNode._cache._b1AccText !== undefined) {
    context.processed = context.processed || new Set();
    context.processed.add(virtualNode);
    return virtualNode._cache._b1AccText;
  }
  ...
  if (b1Memo && !b1Seen) {
    // [a11y-core]
    virtualNode._cache._b1AccText = accessibleName;
  }
 ...
 * @param {VirtualNode} element
 * @param {Object} context
 * @property {Set<VirtualNode>} processed
 * @return {Boolean}
 */
accessibleTextVirtual.alreadyProcessed = function alreadyProcessed(
  ...
  // [a11y-core]
  context.processed = context.processed || new Set();
  if (context.processed.has(virtualnode)) {
    return true;
  }
